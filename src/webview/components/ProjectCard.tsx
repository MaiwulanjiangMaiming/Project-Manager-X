...
function ProjectCard({
  ...
  const isRemote =
    project.remote || ['ssh', 'docker', 'wsl', 'devcontainer', 'codespaces'].includes(project.type);
    ...
    >
      <div className="project-card-content" onClick={isManageMode ? undefined : handleContentClick}>
        <div className="project-card-header">
          {isManageMode && (
            <div
              className="project-manage-checkbox"
              onClick={(e) => {
                e.stopPropagation();
                onToggleSelection?.(project.id);
              }}
          ...
          <span className="project-icon" data-tip={iconInfo.label}>
            {iconInfo.paths ? (
              <svg viewBox={iconInfo.viewBox || '0 0 16 16'} fill="currentColor">
                {iconInfo.paths.map((d, i) => (
                  <path key={i} d={d} />
                ))}
              </svg>
            ) : (
          ...
          <>
            <div className="project-card-meta">
              ...
              <span className="project-date">
                {formatLastOpened(project.lastOpened) || 'Never opened'}
              </span>
        ...
        <ContextMenu
          ...
          items={[
            { icon: '▶', label: 'Open', onClick: () => onOpenProject(project.id) },
            {
              ...
              onClick: () => onOpenInNewWindow(project.id),
            },
